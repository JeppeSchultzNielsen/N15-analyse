...
vector<double> AlphacmEfitterCoin(string in, double factor, double gammaLoss){
    ...
    auto cmEnergy = beamVector[3] - gammaLoss;
    ...
    for (Int_t i = 0; i < entries; i++) {
        ...
        for (Int_t j = 0; j < mul; j++) {
            if(canBeAlpha == 0) continue;
            double_t currentEnergy = cmE[j];
            if(currentEnergy < int(expectedE+0.5)+299 && currentEnergy > (int(expectedE+0.5)-300)) {
    ...
    string histRoot = "alphaAccCalib/cmECoinHists/"+energyString +".root";
