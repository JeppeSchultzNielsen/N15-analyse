...
public:
    ...
    vector<Hit> findCoincidences(vector<Hit> hits){
        ...
        auto len = hits.size();
        vector<Hit> coincidenceHits = {};
        for(int i = 0; i < len-1; i++){
            auto currentHit = hits[i];
            for(int k = i+1; k < len; k++){
                if(currentHit.lVector.Vect().Angle(hits[k].lVector.Vect())*TMath::RadToDeg() > 165){
                    if((currentHit.lVector - hits[k].lVector).Mag() < 50){
                        coincidenceHits.emplace_back(hits[i]);
                        coincidenceHits.emplace_back(hits[k]);
                    }
                }
            }
        }
        return coincidenceHits;
    }
